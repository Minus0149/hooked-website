...
/** Chrome for a sub-page: back to the hub, title, then the section's controls. */
function PageShell({
  title,
  sub,
  onBack,
  ...
  return (
    <>
      <header className="topbar">
        <button className="topbar-btn" onClick={onBack} aria-label="Back to settings">
          <IconBack />
        </button>
...
/** One tappable row on the hub that opens a sub-page. */
function NavRow({
  icon,
  iconColor,
  title,
...
export function SettingsScreen({
  ...
  /** which screen of settings is showing */
  page: SettingsPage;
  onOpenPage: (p: SettingsPage) => void;
  isAdmin: boolean;
  onBack: () => void;
  onOpenProfile: () => void;
    ...
    <PageShell title="Gestures" sub="Tune the four swipes to your wrist." onBack={() => onOpenPage("hub")}>
      <div className="settings-row" style={{ cursor: "default" }}>
        ...
        <input
          ...
          onChange={(e) => onSetPrefs({ swipeSensitivity: Number(e.target.value) })}
  ...
  let body: ReactNode;
  if (page === "appearance") body = appearancePage;
  else if (page === "playback") body = playbackPage;
  else if (page === "gestures") body = gesturesPage;

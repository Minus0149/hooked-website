...
// The tools that hunt this pattern have to spell it out, so they are the one
// place it is allowed to appear. Scanning itself is why `scripts/` was never in
// the CI roots, which is how five corrupted comments sat in build-catalog.mjs —
// and skipping only *this* file meant pointing the guard at the repo root
// reported its own siblings, which is the same lie in the other direction.
const SELF = fileURLToPath(import.meta.url);
const TOOLS = new Set(["check-mojibake.mjs", "fix-mojibake.mjs"]);
...
while (stack.length) {
  ...
  if (resolve(cur) === SELF || TOOLS.has(basename(cur))) continue;
  let text;
  try {
